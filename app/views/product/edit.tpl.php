...
<h2>Modifier un produit : <?= $product->getName() ?></h2>

<form action="<?= $router->generate('product-edit', ["productId" => $product->getId()]) ?>" method="POST" class="mt-5">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="picture" name="picture" placeholder="image jpg, gif, svg, png" 
    ...
    <div class="form-group">
        ...
        <select class="custom-select" id="status" name="status" aria-describedby="statusHelpBlock">
            <option value="0" 
            <?php if ($product->getStatus() == 0){ echo "selected"; }?> 
            >Inactif</option>
            <option value="1"
            <?php if ($product->getStatus() == 1){ echo "selected"; }?> 
            >Actif</option>
        </select>
